const HISTORYS = [
  {
    key: "403",
    name: "포텐데이 403",
    organization: "비사이드",
    date: "2024-03 ~ 2024-04",
    link: "https://bside.best/potenday/recruit?gad_source=1&gclid=CjwKCAjwufq2BhAmEiwAnZqw8h38a8alm5shmREqTaopGVFbkg9bX_YTRJEmRJJpPqZnhVVl_7NjHBoCsFoQAvD_BwE",
  },
  {
    key: "wonted",
    name: "프리온보딩 프론트엔드 챌린지",
    organization: "원티드",
    link: "https://www.wanted.co.kr/events/pre_ob_open",
  },
  {
    key: "codeit",
    name: "코드잇 스프린트 심화 프론트엔드 챌린지 2기",
    organization: "코드잇",
    date: "2024-07 ~ 2024-09",
    link: "https://www.codeit.kr/",
  },
  {
    key: "ossca",
    name: "2024 오픈소스 컨트리뷰션 아카데미 [체험형-2차]",
    organization: "Open UP",
    date: "2024-09 ~ 진행중",
    link: "https://github.com/ant-design-korea/ant-design",
  },
]

export { HISTORYS }
